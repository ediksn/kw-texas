...
describe('Testing redux login', () => {
  ...
  it('testing store', () => {
    ...
    const mockAction: LogInProduceProps = expectedActions.find(action => action.type === LOG_IN_SUCCESS) || {
      type: 'error'
    }
    ...
    expect(loginReducer(oldStore.login, mockAction)).toEqual(expectedStore.login)
  })
})
