...
const actionCreators = {
  uploadVideo: (uri: string, title: string, description: string) => async (dispatch: AppDispatch) => {
    const { POST_VIDEOS, POST_VIDEOS_SUCCESS, POST_VIDEOS_FAILURE } = UPLOAD_VIDEOS_TYPES
    dispatch({ type: POST_VIDEOS })
    try {
      const response = await uploadVideoService.uploadVideos(uri, title, description)
      dispatch({
