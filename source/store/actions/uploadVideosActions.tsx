...
const actionCreators = {
  uploadVideo:
    (videoUrl: any, title: string, description: string, timeRecorded: number) => async (dispatch: AppDispatch) => {
      const { POST_VIDEOS, POST_VIDEOS_SUCCESS, POST_VIDEOS_FAILURE } = UPLOAD_VIDEOS_TYPES
      dispatch({ type: POST_VIDEOS })
      try {
        const response = await uploadVideoService.uploadVideos(videoUrl, title, description, timeRecorded)
        dispatch({
